...
namespace bank
{
    public partial class _5 : Form
    {
        ...
        private void textBox2_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (int.Parse(textBox1.Text) >= 2000 && int.Parse(textBox2.Text) <= 60)
                {
                    label6.Text = Convert.ToString((long.Parse(textBox1.Text) * long.Parse(textBox2.Text) * 0.02) + long.Parse(textBox1.Text));
                }
                else
                {
