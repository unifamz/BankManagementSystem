...
namespace bank
{
    public partial class _15 : Form
    {
        ...
        private void textBox2_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (int.Parse(textBox1.Text) >= 2000 && int.Parse(textBox2.Text) <= 180)
                {
                    label6.Text = Convert.ToString((int.Parse(textBox1.Text) * int.Parse(textBox2.Text) * 0.07) + int.Parse(textBox1.Text));
                }
                else
                {
